...
heatmaps=function(datos, barra_secundaria=TRUE, barra_secundaria_nombre="Location", reorganizar_Col=TRUE, modelo_orden="dist"){
  ...
  if (barra_secundaria==TRUE & reorganizar_Col==TRUE){
    ...
    heatmaply::heatmaply(data_log, colors = colores, Colv = reorganizar_Col,
                         ...
                         col_side_colors = nombres_sin_numero,
                         subplot_heights = c(0.15,0.04,0.8),
                         subplot_widths = c(0.85,0.14),
                         limits=c(-rango, rango),
                         file=NULL)
    
  } else if (barra_secundaria==FALSE & reorganizar_Col==TRUE){
    
    heatmaply::heatmaply(data_log, colors = colores, Colv = reorganizar_Col,
                         distfun = modelo_orden,
                         showticklabels = c(T, T),
                         column_text_angle = 90,
                         subplot_heights = c(0.15,0.84),
                         subplot_widths = c(0.85,0.14),
                         limits=c(-rango, rango),
                         file=NULL)
    
  } else if (barra_secundaria==TRUE & reorganizar_Col==FALSE){
  ...
  } else if (barra_secundaria==FALSE & reorganizar_Col==FALSE){
    ...
    message("._____________________________________________________________________________________________.
